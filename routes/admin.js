	...
	const validation = await query
		...
		.then((drivers) => {
			...
			const schema = Joi.object().keys({
				name: {
					first: Joi.string().pattern(/^[A-Za-z]+$/).required(),
...
router.put('/update-vehicle-type/:id', async (req, res) => {
	...
	const { error, value } = await validateVehicleType(req.body, true, req.params.id);

	if (error) return res.status(400).json({ error: error });

	vehicleTypeModel.findByIdAndUpdate(req.params.id, value, { new: false }).exec().then((vehicleType) => {
		//checking if given id does not exist in the database
		if (!vehicleType) return res.status(400).json({ error: 'Vehicle type not found' });
		return res.status(200).json({ message: 'Vehicle Type updated successfully' });
	});
});
...
router.put('/update-vehicle/:id', async (req, res) => {
	...
	vehicleModel
		...
		.exec()
		.then((vehicle) => {
			if (!vehicle) return res.status(400).json({ error: 'Invalid vehicle id provided' });
...
router.put('/repair-vehicle/:id', (req, res) => {
	...
	vehicleModel
		...
		.then((vehicle) => {
			...
			if (!vehicle) return res.status(400).json({ error: 'Invalid id provided' });
...
async function validateVehicleType(vehicleType, isUpdate = false, id = null) {
	let query = vehicleTypeModel.find().where('type').equals(vehicleType.type.toLowerCase());
...
async function validateVehicle(vehicle, isUpdate = false, id = null) {
	const validation = await vehicleTypeModel
		...
		.then(async (type) => {
			...
			let query = vehicleModel.where('license_plate').equals(vehicle.license_plate.trim().toUpperCase());
...
router.post('/update-depot/:depotId', async (req, res) => {
	...
	depotModel.findByIdAndUpdate({ _id: id }, { $set: req.body }).then((result) => {
		//checking if given id does not exist in the database
...
router.post('/update-storekeeper/:userId', async (req, res) => {
	...
	userModel.findByIdAndUpdate({ _id: id }, { $set: req.body }).then((result) => {
		//checking if given id does not exist in the database
		if (!result) return res.status(400).json({ error: 'Store keeper not found' });
		return res.status(200).json({ message: 'Store-keeper updated successfully' });
	});
});
